<style lang='sass'>
  @import '../../scss/layout';
  .sales-record{
    padding-bottom: 64px;
    .search{
      height: 36px;
      padding: 10px 50px 10px 10px;
      background-color: #f0f0f0;
      position: relative;
      .input-box{
        box-sizing: border-box;
        width: 100%;
        padding: 0 28px 0 10px;
        border: 1px solid #e6e6e6;
        border-radius: 5px;
        background-color: #fff;
        position: relative;
        .input-text{
          width: 100%;
          padding: 10px 0;
          background: none;
          border: none;
          font-size: 12px;
          color: #999;
        }
      }
      .search-btn{
        width:28px;
        height: 36px;
        background: url(data:image/png;base64,) center center no-repeat;
        background-size: 12px auto;
        position: absolute;
        top: 0;
        right: 0;
        z-index: 1;
      }
      .filtrate-btn{
        width: 50px;
        height: 100%;
        line-height: 56px;
        font-size: 14px;
        color: #333;
        text-align: center;
        position: absolute;
        right: 0;
        top: 0;
        z-index: 1;
      }
      .filtrate-date{
        width: 100%;
        background-color: rgba(0, 0, 0, 0.36);
        position: fixed;
        top: 56px;
        left: 0;
        bottom: 0;
        z-index: 1;
        .form-info{
          width: 100%;
          padding: 20px 0 18px;
          background-color: #fff;
          .info-box{
            padding: 0 10px;
            margin-bottom: 20px;
          }
          .info-radio{
            padding: 0 10px;
            margin-bottom: 20px;
            margin-left:20%;
          }
        }
        .box-title, .box-text{
          height: 30px;
          line-height: 30px;
          color: #333;
        }
        .box-title, .radio-text{
           height: 30px;
           color: #333;
         
         }
        .input-text{
          box-sizing: border-box;
          width: 46%;
          height: 30px;
          border: 1px solid #e5e5e5;
          text-align: center;
          color: #999;
          float: left;
        }
        .date-to{
          float: left;
          width: 8%;
          text-align: center;
        }
      }
      .btns{
        font-size: 0;
        text-align: center;
        .btn{
          width: 100px;
          height: 30px;
          margin: 0 20px;
        }
      }
    }
    .tab-cut{
      border-bottom: 1px solid #f0f0f0;
      background-color: #fff;
      font-size: 0;
      .tab{
        display: inline-block;
        width: 33.3%;
        height: 42px;
        line-height: 42px;
        font-size: 14px;
        color: #333;
        text-align: center;
        position: relative;
        &.active:after{
          content: "";
          display: block;
          width: 66%;
          height: 3px;
          background-color: #ff5151;
          position: absolute;
          left: 0;
          right: 0;
          bottom: 0;
          margin: 0 auto;
        }
      }
    }
    .data-list{
      padding: 0 10px;
      background-color: #fff;
      .data-total{
        padding-top: 10px;
        line-height: 16px;
        font-size: 12px;
        color: #999;
      }
       .radspan{
            display: block;
    		vertical-align: middle;
    		width: 20px;
    		height: 20px;
    		-webkit-appearance: radio;
    		background-color: transparent;
    		border: 0;
    		outline: 0 !important;
    		line-height: 20px;
    		color: #d8d8d8;
        }
        .spandiv{
         margin-left:20%;
        }
        .spandivs{
         margin-left:27%;
        }
      .data-box{
        overflow: hidden;
        padding: 10px 0 5px;
        border-bottom: 1px solid #e9e9e9;
        .product-img{
          width: 90px;
          height: 90px;
          margin-right: 15px;
          border: 1px solid #e9e9e9;
          background-position: center center;
          background-color: #fff;
          background-size: auto 80%;
          background-repeat: no-repeat;
          float: left;
        }
        .pay{
          height: 40px;
          margin-right: 15px;
          border: 1px solid #fff;
          background-position: center center;
          background-color: #fff;
          background-size: auto 80%;
          background-repeat: no-repeat;
          float: right;
          z-index:10;
          margin-top:8%;
         
    	}
    	.payoff{
          height: 40px;
          margin-right: 15px;
          border: 1px solid #fff;
          background-position: center center;
          background-color: #fff;
          background-size: auto 80%;
          background-repeat: no-repeat;
          float: right;
          z-index:10;
          margin-top:8%;
    	 }
    	 
        .form-info{
          z-index:1;
          font-size: 12px;
          color: #333;
          overflow: hidden;
          .info-box{
            margin-bottom: 5px;
          }
          .info-radio{
            margin-bottom: 5px;
          }
          .box-title{
            width: 60px;
          }
        }
      }
    }
  }
</style>

<template lang='jade'>
  .wrapper.sales-record
    form.search(@submit.prevent='initRecordData(true)')
      .input-box
        input.input-text.keywords(
          placeholder="搜索商品",
          v-model="productName"
        )
        a.search-btn(@click="initRecordData(true)")
      a.filtrate-btn(
        @click='toggle = !toggle'
      ) 筛选
      .filtrate-date(v-show='toggle')
        .form-info
          .info-box
            span.box-title 扫码时间
            .box-text
              input.input-text(
                type="date" placeholder="开始时间",
                v-model="startTime"
              )
              span.date-to 至
              input.input-text(
                type="date" placeholder="结束时间",
                v-model="endTime"
              )
           .info-radio
            input.radspan(
               style="-webkit-appearance: radio;",
               type="radio" value="1"  name="pay" checked="checked" id="pay",
               v-model = "paymentstate"
             )
            label.spandiv(style="padding:5px;" for="pay") 已支付
            
              input.radspan(
               style="-webkit-appearance: radio;margin-left:27%",
               type="radio" value="0" name="pay" id="payoff",
               v-model = "paymentstate"
             )
            label.spandivs(style="padding:5px;" for="payoff") 未支付
            
          .info-box.btns
            input.btn.btn-white(style="margin-top:3%;margin-left:16%;"
             					type="button" value="取消", @click='toggle = !toggle')
            input.btn.btn-red(
              type="button" value="确定",
              @click='changeTimeType(4)'
            )
    nav.tab-cut#navContainer
      a.tab(
        @click='changeTimeType(1)',
        :class='{active: timeType==1}'
      ) 今天
      a.tab(
        @click='changeTimeType(2)',
        :class='{active: timeType==2 }'
      ) 近7天
      a.tab(
        @click='changeTimeType(3)',
        :class='{active: timeType==3 }'
      ) 近30天

    dl.data-list(
      v-if='items.length > 0'
    )
      dt.data-total 共有{{resultTotal}}条记录
      dd.data-box(v-for='item in items')
        .product-img(:style='{backgroundImage:"url("+item.proImage+")"}')
        dl.form-info.justify   
          img.pay(:src="pay" v-if="item.paymentState !='0' ")
          img.payoff(:src="payoff" v-if="item.paymentState =='0' ")
          dt.info-box {{item.proName}}
          dd.info-box
            span.box-title 产品编码：
            .box-text {{item.proNumber}}
          dd.info-box
            span.box-title 费用(元)：
            .box-text {{item.price}}
          dd.info-box
            span.box-title 数量(瓶)：
            .box-text {{item.nub}}
          dd.info-box    
            span.box-title 扫码时间：
            .box-text {{item.cTime | formatDate}}
          dd.info-box 
            span.box-title(v-if="item.paymentState !='0' ") 支付时间：
            .box-text {{item.paymenttime | formatDate}}
    p.not-found(v-else) 暂无数据!
    promoter-nav
</template>

<script lang='babel'>
  import {
    promoterRecordURL,
  } from '../../api.js';
  import PromoterNav from './nav.vue';
  export default {
    components: {
      PromoterNav,
    },
    data() {
      return {
        banner: '/dist/images/banner.jpg',
        pay:'/dist/images/pay.png',
        payoff:'/dist/images/payoff.png',
        page: 0,
        pageSize:0,
        pageTotal:0,
        resultTotal:0,
        timeType:1,
        ajaxFlag:false,
        items: [],
        productName: '',
        startTime: '',
        endTime: '',
        toggle: false,
        paymentstate:'',
       
      }
    },
    methods: {
      changeTimeType(type){
        this.toggle = false;
        this.timeType = type;
        this.initRecordData(true);
      },

      resetInit(){
        this.page = 0;
        this.pageSize = 0;
        this.pageTotal = 0;
        this.resultTotal = 0;
        this.ajaxFlag = false;

        self.document.body.scrollTop = 0;
        self.document.removeEventListener('scroll', this.next);
      },
      next(){
        const scrollHeight = self.document.body.scrollHeight;
        const scrollTop    = self.document.body.scrollTop;
        if (self.innerHeight + scrollTop + 50 > scrollHeight && !this.ajaxFlag) {
          if (this.page < this.pageTotal) {
            this.initRecordData();
          }
        }
      },
      initRecordData(resetFlag) {
        if(resetFlag){
          this.resetInit();
        }

        const url = promoterRecordURL;
        const salesmanId = self.qrcode.salesmanId;

        this.ajaxFlag = true;

        self.common.get(url, {
          params:{
            productName:this.productName,
            timeType: this.timeType,
            startTime:this.startTime ? new Date(this.startTime).getTime().toString() : this.startTime,
            endTime:this.endTime ? new Date(this.endTime).getTime().toString() : this.endTime,
            page: this.page+1,
            paymentstate:this.paymentstate,
            qyuuid:self.qrcode.qyuuid,
          },
          callback: function(json) {
            this.ajaxFlag = false;
            if (json.code === 200) {
              this.page = json.data.page;
              this.paymentState = json.data.paymentState;
              if(this.page == 1){
                this.items = json.data.result;
                this.resultTotal = json.data.resultTotal;
                this.pageSize = json.data.pageSize;
                this.pageTotal = json.data.pageTotal;
                if(this.pageTotal > 1){
                  self.document.addEventListener('scroll', this.next);
                }
              } else {
                this.items = this.items.concat(json.data.result);
              }

              if(this.pageTotal - this.page <= 0){
                self.document.removeEventListener('scroll', this.next);
              }
            } else {
              self.toast(json.msg);
            }
          }.bind(this)
        });
      },
    },
    mounted() {
      this.initRecordData(true);
    },
    beforeDestroy() {
      // 在销毁的时候, 一定要先卸载当前组件的 scroll 事件, 不然的话, 在所有组件下都会触发当前组件的 scroll 事件
      self.document.removeEventListener('scroll', this.next);
    }
  }
</script>
